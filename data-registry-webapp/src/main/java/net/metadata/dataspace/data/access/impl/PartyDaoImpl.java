...
public class PartyDaoImpl extends JpaDao<Party> implements PartyDao, Serializable {
    ...
    @Override
    @SuppressWarnings("unchecked")
    public List<Party> getAllPublishedBetween(Date fromDate, Date untilDate) {
        Query query = entityManagerSource.getEntityManager().createQuery("SELECT o FROM Party o WHERE o.published IS NOT NULL AND o.updated BETWEEN :fromDate and :untilDate ORDER BY o.updated");
