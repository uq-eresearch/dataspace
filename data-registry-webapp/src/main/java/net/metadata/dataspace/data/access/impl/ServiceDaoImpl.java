...
public class ServiceDaoImpl extends JpaDao<Service> implements ServiceDao, Serializable {
    ...
    @Override
    @SuppressWarnings("unchecked")
    public List<Service> getAllPublishedBetween(Date fromDate, Date untilDate) {
        Query query = entityManagerSource.getEntityManager().createQuery("SELECT o FROM Service o WHERE o.published IS NOT NULL AND o.updated BETWEEN :fromDate and :untilDate ORDER BY o.updated");
